...
def microwave(device_demand, routine, mininterval, cost, hh_threshold, test):
    # general microwave inputs
    ...
    # inputs especially for microwave optimsiation using elimination:
    elim_thres = 11                              # how much effort it takes to eliminate device usage

    # inputs especially for microwave optimsiation using minimisation 1:
    mini1_thres = 9                             # how much effort it takes to minimise device usage
    mini1_saving = 0                            # if type duration, what percentage will be saved (0 nothing - 100)
    mini1_cyclemax = 1                          # if type cycle, what is the maximum amount of cycles...
    mini1_time = 1440                           # if type cycle, ...within a given time frame
    ...
    if hh_threshold >= mini1_thres:          # apply minimisation optimisations if effort threshold >= mini threshold
        optimised_demand = mn.minimise(optimised_demand, routine, mealappliance, devicetype, mininterval,
                                       mini1_saving, mini1_cyclemax, mini1_time)
...
def test0():                    # test shifting
    ...
    device_demand = appliancedemand[0][9]
    variation = microwave(device_demand, routine_, min_interval, cost_, threshold, testing_)
