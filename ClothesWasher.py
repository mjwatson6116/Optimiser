...
def clotheswasher(device_demand, routine, mininterval, cost, hh_threshold, shifting, test):
    # general clothes washer inputs
    ...
    # inputs especially for clothes washer optimsiation using minimisation 1:
    ...
    # inputs especially for clothes washer optimsiation using shifting 1:
    ...
    # inputs especially for clothes washer optimsiation using shifting 2:
    ...
    # inputs especially for clothes washer optimsiation using shifting 3:
    ...
    # ****** Shifting 1 ******
    if hh_threshold >= shift1_thres and shifting == 1:          # apply shifting optimisation if shifting on & threshold >= shift thres
    ...
    # ****** Shifting 2 ******
    elif hh_threshold >= shift2_thres and shifting == 1:        # apply shifting optimisation if shifting on & threshold >= shift thres
        optimised_demand = sh.shifter(optimised_demand, routine, mealappliance, devicetype, mininterval, shift2_time,
                                      cost, test)                     # return optimised device usage
    # ****** Shifting 3 ******
    elif hh_threshold >= shift3_thres and shifting == 1:        # apply shifting optimisation if shifting on & threshold >= shift thres
    ...
    if test == 1 and hh_threshold >= shift1_thres:
        ...
        print " Opti Shif1 Demand: (" + str(shift2_thres) + ") (" + str(len(optimised_demand)) + ") " + str(optimised_demand)
    elif test == 1 and hh_threshold >= shift3_thres:
        print " Opti Shif1 Demand: (" + str(shift3_thres) + ") (" + str(len(optimised_demand)) + ") " + str(optimised_demand)
    elif test == 1:
        print " Opti Shif1 Demand: (" + str(shift3_thres) + ") (" + str(len(optimised_demand)) + ") " + str(optimised_demand)
...
def test0():                    # test shifting
    ...
    cost_ = all_billing[1]
    expected_ = []
    appliancedemand = ida.deviceimport(housedata, min_interval, csv_directory, "No", "No", "Yes", "No", "No", "No",
                                       "No", "No", "No", "No", "No", "No", "No", "No")
    device_demand = appliancedemand[0][2]
    variation = clotheswasher(device_demand, routine_, min_interval, cost_, threshold, shifting_, testing_)
...
def test1():
    ...
    for i in np.arange(7, 8):
        ...
        vari = clotheswasher(device_data, routine_, min_interval, billing_info[1], hh_threshold, shifting_, 0)
    ...
    plt.title('Impact of Behavioural Change on Clothes Washer Use (Data ID:' + str(dataid) + ")")
    plt.show()
